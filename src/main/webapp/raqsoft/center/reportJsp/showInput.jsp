...
<%
	...
	if (scroll==null || scroll.length()==0 || scroll == "0") scroll = "no";
	else scroll="yes";
	if(paged == "0") paged = "no";
	else paged = "yes";
	com.raqsoft.center.Config cfg = com.raqsoft.center.Center.getConfig( application );
	String reportFileHome = new com.raqsoft.center.Config(request.getServletContext()).getFileRoot();
	...
	com.raqsoft.center.User user = (com.raqsoft.center.User)(session.getAttribute("userObj"));
	String userName = "";
	if(user != null){
	...
	String resultPage = "showInput.jsp?sht=" + URLEncoder.encode( input, "UTF-8" ) + "&noDfx="+noDfx+"&dataFile=" + URLEncoder.encode( dataFile, "UTF-8" ) + "&fileType=" + fileType+"&sgid="+sgid;
