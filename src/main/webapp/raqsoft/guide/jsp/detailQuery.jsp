...
if (dataSource.length()>0) {
	DQLTableFilter filter = new DQLTableFilter("default",dataSource);
	filters.add(filter);
	if (vsb.length()>0) filter.setVsb(vsb);
	//filter.getFilters().put("雇员","${T}.雇员='2'"); //用户可以自己补充表的过滤条件
	filter.getParamValues().put("雇员","6");//设置过滤条件里的参数值（宏值）
}
...
<script>
	...
	guideConf.outerConditionID="default";
	*/
