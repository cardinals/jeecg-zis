...
function refreshStatus(dataSet,finishFunc,container) {
	...
	var callback = function(data) {
		...
		if (data.over == 1) {
			...
			if (data.loadedRow == 0) {
				container.html(resources.guide.js106.replaceAll("{0}",dataSet.name));
			} else {
	...
	,refresh : function(noCalc, noRefreshDialog) {
		...
		if (conf.type == 1) { //自定义报表
			...
			//if (conf.lefts.length == 0) 
				div2.append("<div id='confHints' style='text-align: center; margin:3px;padding:3px;color:lightgray;'>"+resources.guide.js126+"</div>");
	...
	,editConfItem : function(name){
		...
		if (confField._fieldType=='aggr') {
			...
			*/
			...
			d3.append("<div class=''>"+resources.guide.js148+"</div>");
	...
	,queryData : function(conf) {
		...
		if (dataSet.type == 6){
			...
			$.ajax({
				...
				async : false,
				url: contextPath + "/DLServletAjax?d="+new Date().getTime(),
				data: {action:2,oper:'queryDqlData',reportId:conf.reportId,dqlSegments:dql,dataSource:dataSet.dataSource,maxDataSize:guideConf.maxDataSize,outerConditionID:guideConf.outerConditionID},
				success: function(data){
					if (data.indexOf('error:')==0) {
						alert(data.substring(6));
						return;
