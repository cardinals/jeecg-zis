...
//无保存按钮的弹框
function add_nosave(title,addurl,gname,width,height) {//createwindow_nosave
	gridname=gname;
	createwindow_nosave(title, addurl,width,height);
}
/**
 * 添加事件打开窗口
 * @param title 编辑框标题
...
function shenheALLSelect(title,url,gname) {
    ...
    if (rows.length > 0) {
    	...
    	$.dialog.confirm('你确定审核所有数据吗?', function(r) {
		   if (r) {
...
/**
 * 创建添加或编辑窗口_无保存按钮的弹框========20181101
 * 
 * @param title
 * @param addurl
 * @param saveurl
 */
function createwindow_nosave(title, addurl,width,height) {
	width = width?width:700;
	height = height?height:400;
	if(width=="100%" || height=="100%"){
		width = window.top.document.body.offsetWidth;
