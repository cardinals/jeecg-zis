...
<div class="easyui-layout" fit="true">
  ...
  <t:datagrid name="pasAstStaffrowList" checkbox="true"  pagination="true" fitColumns="true" 
  title="员工互评(资产)" actionUrl="pasAstStaffController.do?datagrid" onLoadSuccess="loadSucessFun" idField="id"  queryMode="group">
    <t:dgCol title="主键"  field="id"  hidden="true" ></t:dgCol>
    <t:dgCol title="评价人"  field="appraiser"  hidden="true" ></t:dgCol>
    <t:dgCol title="被评价人"  field="goalPerson" rowspan="4" queryMode="single"    width="80"></t:dgCol>
    <t:dgCol title="部门"  field="goalPersonDept" rowspan="4" queryMode="single"   width="90"></t:dgCol>
    ...
    <t:dgCol title="<b>合作效率(该项满分20分)</b>"  field="coopEff" queryMode="single"  dictionary="pas_s20" extendParams="editor:'combobox'"  ></t:dgCol>
    <t:dgCol title="<b>合作态度(该项满分20分)</b>"  field="coopAtti" queryMode="single"  dictionary="pas_s20" extendParams="editor:'combobox'" ></t:dgCol>
    ...
    <t:dgCol title="合计"  field="totalScore" newColumn="true" rowspan="4" queryMode="single"   width="50"></t:dgCol>
   	<!-- start -->
	  ...
	  <t:dgCol align="left"   title="" ></t:dgCol> 
	  <!-- end -->
    <t:dgToolBar title="提交"  icon="icon-save" url="pasAstStaffController.do?saveRows" funname="saveData"></t:dgToolBar>
 ...
 //更新数据
	...
	function saveData(title,addurl,gname){
	    ...
	    $.messager.confirm("确认", "确定要提交吗？提交后不可修改 ! ", function (r) {
			if (r) {
	    		...
	    		for(var i=0;i<rows.length;i++){
	    			for(var d in rows[i]){
	    				result["pasAstStaffList["+i+"]."+d]=rows[i][d];
	    			}
	    		}
	    		$.ajax({
	    			url:"<%=basePath%>/"+addurl,
	...
	function loadSucessFun(data){
		var gname = "pasAstStaffrowList";
		 $.ajax({
				url:"<%=basePath%>/"+"pasAstStaffController.do?checkConfirm",
				type:"post",
				dataType:"json",
				success:function(data){
					//tip(data.msg);
