...
<div class="easyui-layout" fit="true">
  ...
  <t:datagrid name="pasStaffLeadList" checkbox="true" pagination="true" fitColumns="false" title="下级对上级领导评分" actionUrl="pasStaffLeadController.do?datagrid" idField="id" fit="true"  onLoadSuccess="loadSucessFun" queryMode="group">
...
<!-- end -->
   ...
   <t:dgDelOpt title="删除" url="pasStaffLeadController.do?doDel&id={id}" urlclass="ace_button"  urlfont="fa-trash-o"/>
   <t:dgToolBar title="录入" icon="icon-add" url="pasStaffLeadController.do?goAdd" funname="add"></t:dgToolBar>
   <t:dgToolBar title="编辑" icon="icon-edit" url="pasStaffLeadController.do?goUpdate" funname="update"></t:dgToolBar>
   <t:dgToolBar title="批量删除"  icon="icon-remove" url="pasStaffLeadController.do?doBatchDel" funname="deleteALLSelect"></t:dgToolBar>
   <t:dgToolBar title="查看" icon="icon-search" url="pasStaffLeadController.do?goUpdate" funname="detail"></t:dgToolBar>
   <t:dgToolBar title="导入" icon="icon-put" funname="ImportXls"></t:dgToolBar> --%>
   <t:dgToolBar  title="提交"  icon="icon-save" url="pasStaffLeadController.do?saveRows" funname="saveData"></t:dgToolBar>
 ...
 <script src = "webpage/generator/pas/passtafflead/pasStaffLeadList.js"></script>		
...
//------------------------------------------start---------------------------------------------
	...
	function saveData(title,addurl,gname){
		...
		$.messager.confirm("确认", "确定要提交吗？提交后不可修改 ! ", function (r) {
	        if (r) {
	    		...
	    		for(var i=0;i<rows.length;i++){
	    			for(var d in rows[i]){
	    				result["pasStaffLeadList["+i+"]."+d]=rows[i][d];
	    			}
	    		}
	    		$.ajax({
	    			url:"<%=basePath%>/"+addurl,
	...
	function loadSucessFun(data){
		var gname = "pasStaffLeadList";
		 $.ajax({
				url:"<%=basePath%>/"+"pasStaffLeadController.do?checkConfirm",
				type:"post",
				dataType:"json",
				success:function(data){
					//tip(data.msg);
					if(!data.attributes.initStatus){//还没有评过分
						...
						/* 	var editors = $('#'+gname).datagrid('getEditors', index); 
							//获取控件对象  动态设置必选验证box
							for(var i=0;i<4;i++){
							 commentEditor= editors[i].target;
							 commentEditor.combobox({
...
//导入
function ImportXls() {
	openuploadwin('Excel导入', 'pasStaffLeadController.do?upload', "pasStaffLeadList");
}

//导出
function ExportXls() {
	JeecgExcelExport("pasStaffLeadController.do?exportXls","pasStaffLeadList");
}

//模板下载
function ExportXlsByT() {
	JeecgExcelExport("pasStaffLeadController.do?exportXlsByT","pasStaffLeadList");
}
