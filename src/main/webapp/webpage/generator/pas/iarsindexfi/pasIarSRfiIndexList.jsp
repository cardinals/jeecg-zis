...
<div class="easyui-layout" fit="true">
  ...
  <t:datagrid name="pasIarSRfiIndexrowList" checkbox="true"  pagination="true" fitColumns="true" 
  title="投研交流满意度-研究对投资" actionUrl="pasIarSRfiIndexController.do?datagrid"  onLoadSuccess="loadSucessFun"  idField="id"  queryMode="group">
    <t:dgCol title="主键"  field="id"  hidden="true" ></t:dgCol>
    <t:dgCol title="评价人"  field="appraiser"  hidden="true" ></t:dgCol>
    <t:dgCol title="被评价部门"   field="goalDept"  rowspan="3" align="center" queryMode="single"   ></t:dgCol>
    <t:dgCol title="<b>投研交流满意度</b>"  field="iarCmuIdx"  queryMode="single" align="left"  dictionary="pas_s100" extendParams="editor:'combobox'" ></t:dgCol>
    <t:dgCol title="评分说明"  field="comment" newColumn="true" rowspan="3"  align="center"  queryMode="single"   extendParams="editor:'text'" width="100"></t:dgCol>
  	
  	 <t:dgCol title="积极与研究人员进行协调和沟通;配合股票池管理"    ></t:dgCol>
  	<t:dgToolBar title="提交" icon="icon-save" url="pasIarSRfiIndexController.do?saveRows" funname="saveData"></t:dgToolBar>
...
//更新数据
	function updateData(title,addurl,gname){
		...
		for(var i=0;i<rows.length;i++){//所有行
			...
			//var total1 = parseInt(rows[i].professionalSkill)+parseInt(rows[i].coopEff)+parseInt(rows[i].coopAtti)+parseInt(rows[i].coopOutcome);
			//alert(total1);
			$('#'+gname).datagrid('updateRow',{index:index1,row:{iarCmuIdx:iarCmuIdx}});
		}
 ...
 //变量初始化
	 ...
	 $.messager.confirm("确认", "确定要提交吗？提交后不可修改 ! ", function (r) {
		if (r) {
	    		...
	    		for(var i=0;i<rows.length;i++){
	    			for(var d in rows[i]){
	    				result["pasIarSRfiIndexList["+i+"]."+d]=rows[i][d];
	    			}
	    		}
	    		$.ajax({
	    			url:"<%=basePath%>/"+addurl,
	...
	function loadSucessFun(data){
		showLoading();
		var gname = "pasIarSRfiIndexrowList";
		 $.ajax({
				url:"<%=basePath%>/"+"pasIarSRfiIndexController.do?checkConfirm",
				type:"post",
				dataType:"json",
				success:function(data){
					//tip(data.msg);
					if(!data.attributes.initStatus){
						...
						for(var i=0;i<rows.length;i++){
							...
							commentEditor.bind("click",function(event){
								...
								createWindowOnComment('评语编辑', 'pasIarSRfiIndexController.do?showDialogPage&rowIndex='+index,400,180,target);
							});
						}
					}
					hideLoading();
