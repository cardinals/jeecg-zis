...
//          sql = sql.replace("${" + key + "}", "'"+String.valueOf(params.get(key))+"'");
	...
	private void fillInsertSysVar(String tableName,Map<String, Object> data) {
		 ...
		 * 0928
		 */
		if(data.containsKey(DataBaseConstant.SYS_USER_ID)
				||getAllFieldByTableName(tableName).containsKey(DataBaseConstant.SYS_USER_ID)){
			data.put(DataBaseConstant.SYS_USER_ID, ResourceUtil.getUserSystemData(DataBaseConstant.SYS_USER_ID));
		}
	...
	private Map<String, Object> minidaoReplaceExtendSqlSysVar(Map<String, Object> data){
		...
		//0928
		data.put("sys."+DataBaseConstant.SYS_USER_ID, ResourceUtil.getUserSystemData(DataBaseConstant.SYS_USER_ID));
	...
	private String replaceExtendSqlSysVar(String sql){
		sql = sql.replace("#{sys."+DataBaseConstant.SYS_USER_CODE_TABLE+"}", ResourceUtil.getUserSystemData(DataBaseConstant.SYS_USER_CODE))
				...
				.replace("#{sys."+DataBaseConstant.SYS_USER_ID+"}", ResourceUtil.getUserSystemData(DataBaseConstant.SYS_USER_ID))
