...
import org.jeecgframework.web.system.util.HTTPUtil;
...
public class LoginController extends BaseController {
	...
	@Autowired
	private TSSmsServiceI tSSmsService;//测试短信用的
	 ...
	 * @throws Exception
	 */
	@RequestMapping(params = "oaLogin")
	public String toLoginPage(ModelMap modelMap,HttpServletRequest request,HttpServletResponse response) throws Exception {
		String passUsernameStr = request.getParameter("username");
		String username = EncryptUtil.decrypt(passUsernameStr,ZrjjdanganConstant.pwd_sso);
		TSUser userTemp = new TSUser();
		userTemp.setUserName(username);
		// 用户登录验证逻辑  沿用微信只验证用户名的方式
		TSUser user = userService.WXcheckUserExits(userTemp);
		this.OAcheckuser(username, request);
		String roles = "";
		if (user != null) {
			...
			modelMap.put("NoExitUserid", username);
			return "login/sso_error";
		}
	}
	
	/* 微信企业用户只检查用户名 */
	public AjaxJson OAcheckuser(String userName, HttpServletRequest req) {
		...
		// 用户登录验证逻辑
		TSUser u = userService.WXcheckUserExits(user);
		if (u == null) {
			/*
			 * u = userService.findUniqueByProperty(TSUser.class, "email",
			 * user.getUserName()); if (u == null || u.getPassword()
			 * .equals(PasswordUtil.encrypt(u.getUserName(), u.getPassword(),
			 * PasswordUtil.getStaticSalt()))) {
			 * j.setMsg(mutiLangService.getLang(
			 * "common.username.or.password.error")); j.setSuccess(false);
			 * return j; }
			 */}
			...
			if (oConvertUtils.isEmpty(orgId)) {
				...
				if (orgNum > 1) {
					...
					Map<String, Object> userOrgMap = systemService
							.findOneForJdbc("select org_id as orgId from t_s_user_org where user_id=? limit 1", u.getId());
	 ...
	 * @throws Exception
	 */
	@RequestMapping(params = "oaLogin_old")
	public String toLoginPage_old(ModelMap modelMap,HttpServletRequest request,HttpServletResponse response) throws Exception {
		String passUsernameStr = request.getParameter("username");
		String username = EncryptUtil.decrypt(passUsernameStr,ZrjjdanganConstant.pwd_sso);
		TSUser userTemp = new TSUser();
		userTemp.setUserName(username);
		// 用户登录验证逻辑  沿用微信只验证用户名的方式
		TSUser user = userService.WXcheckUserExits(userTemp);
		this.WXcheckuser(username, request);
		String roles = "";
		if (user != null) {
			List<TSRoleUser> rUsers = systemService.findByProperty(TSRoleUser.class, "TSUser.id", user.getId());
			for (TSRoleUser ru : rUsers) {
	...
	public AjaxJson sendResetPwdMail(String email, HttpServletRequest request) {
		...
		try {
			...
			// 保存重置密码数据信息
			String hql = "from TSPasswordResetkey bean where bean.username = '" + user.getUserName()
					+ "' and bean.isReset = 0 order by bean.createDate desc limit 1";
			...
			contentBuffer.append(
					"style=\"position:relative;font-size:14px;height:auto;padding:15px 15px 10px 15px;z-index:1;zoom:1;line-height:1.7;\" class=\"body\">");
			contentBuffer.append(
			...
			contentBuffer.append("<tbody><tr><td style=\"color:#40AA53;\"><h1 style=\"margin-bottom:10px;\">" + title
					+ "</h1></td></tr>");
			contentBuffer.append(
					"<tr><td style=\"border-left: 1px solid #D1FFD1; padding: 20px 20px 0px; background: none repeat scroll 0% 0% #ffffff; border-top: 5px solid #40AA53; border-right: 1px solid #D1FFD1;\">");
			contentBuffer.append("<p>你好 </p></td></tr>");
			contentBuffer.append(
					"<tr><td style=\"border-left: 1px solid #D1FFD1; padding: 0px 45px 0px; background: none repeat scroll 0% 0% #ffffff; border-right: 1px solid #D1FFD1;\">");
			...
			contentBuffer.append(
					"<tr><td style=\"border-left: 1px solid #D1FFD1; padding: 10px 20px; background: none repeat scroll 0% 0% #ffffff; border-right: 1px solid #D1FFD1;\">");
			...
			contentBuffer.append(
					"<tr><td style=\"border-bottom: 1px solid #D1FFD1; border-left: 1px solid #D1FFD1; padding: 0px 20px 20px; background: none repeat scroll 0% 0% #ffffff; border-right: 1px solid #D1FFD1;\">");
			contentBuffer.append("<hr style=\"color:#ccc;\">");
			String commentUrl = "http://www.jeecg.org";
			contentBuffer.append("<p style=\"color:#060;font-size:9pt;\">想了解更多信息，请访问 <a href=\"" + commentUrl
					+ "\" target=\"_blank\">" + commentUrl + "</a></p>");
			...
			MailUtil.sendEmail(util.readProperty("mail.smtpHost"), email, "邮箱重置密码", contentBuffer.toString(),
	...
	@ResponseBody
	public AjaxJson sendCode(TSUser user, HttpServletRequest req) {
		...
		if(u != null){
			...
			//3,将验证码存入session、数据库中
			HttpSession session = req.getSession();
			System.out.println(verifyCode);
			Date date = new Date();    
			String time = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(date);  
			Timestamp sendTime = Timestamp.valueOf(time);//插入验证码时间
			String smsId =  String.valueOf(new Random().nextInt(899999) + 100000);
			//4,把手机号、验证码存到本地业务数据
			TSSmsEntity tSSmsEntity = new TSSmsEntity();
			tSSmsEntity.setId(smsId);
			tSSmsEntity.setEsReceiver(phone);
			tSSmsEntity.setEsContent(verifyCode);
			tSSmsEntity.setEsSendtime(sendTime);
			tSSmsService.save(tSSmsEntity);
			//5,把手机号、验证码存到本地短信系统数据库，以用来发短信
			//String smsDbKey = "zrfweb";
			final String smsDbKey = "SMS_DB_KEY";
			TSSysParamsEntity tSSysParamsEntity = systemService.findUniqueByProperty(TSSysParamsEntity.class, "citem",smsDbKey);
			String smsDbKeyValue = tSSysParamsEntity.getCValue();
			
			String smsSql = "insert into tb_queue (id,phone,msg) values (?,?,?)";
			String smsSendCode = new StringBuffer("中融基金:").append(verifyCode).toString();
			int smsCount = DynamicDBUtil.update(smsDbKeyValue,smsSql,smsId,phone,smsSendCode);
			if(smsCount <= 0){
				j.setSuccess(false);
			}
			//获取短信验证码时效，传到前台
			int maxSmsAge = MaxSmsAge();
			j.setObj(maxSmsAge);
			//j.setMsg("操作成功");
		}else {
	...
	public AjaxJson checkuser(TSUser user, HttpServletRequest req) {
		...
		if (StringUtils.isNotEmpty(returnURL)) {
			...
			String acquireCode = new String("");//获得的验证码
			boolean isSmsVerifyCode = false;
			int timeDiff = 0;//短信验证码存活的时间
			int maxSmsAge = 0;//短信最大时效
			//判断是否使用短信验证码
			isSmsVerifyCode = isSmsVerifyCode(req);
			if(isSmsVerifyCode){//使用短信验证码
				Map<String,Object> sMSmaps = smsTimeDiff(user);
				acquireCode = String.valueOf(sMSmaps.get("acquireCode"));
				timeDiff = Integer.parseInt(String.valueOf(sMSmaps.get("interval")));
				maxSmsAge = MaxSmsAge();
			}else{
				acquireCode = (String) session.getAttribute("randCode");//图片验证码
			}
		 //Long seconds = DateUtil.getDifferenceSeconds(sendCodeTime, new Date());
/*		if (StringUtils.isEmpty(randCode)) {
			j.setMsg(mutiLangService.getLang("common.enter.verifycode"));
			j.setSuccess(false);
		//} else if (!randCode.equalsIgnoreCase(String.valueOf(session.getAttribute("randCode")))) {//图片验证码
		//} else if (!randCode.equalsIgnoreCase(String.valueOf(session.getAttribute("SMSCode")))) {//session中短信验证码
		//} else if (!randCode.equalsIgnoreCase(String.valueOf(smsCode))) {//数据库中的短信验证码
			
		}else if (!randCode.equalsIgnoreCase(String.valueOf(acquireCode))) {//图片验证码或者短信验证码
			j.setMsg(mutiLangService.getLang("common.verifycode.error"));
			j.setSuccess(false);
		}else if(isSmsVerifyCode&&timeDiff>maxSmsAge) {//如果是外网，且超时
			j.setMsg("短信验证码超时");
			j.setSuccess(false);
		}*/
		/*else if (isInBlackList(IpUtil.getIpAddr(req))) {
			...
			if (u == null) {
				...
				if (u == null || u.getPassword()
						.equals(PasswordUtil.encrypt(u.getUserName(), u.getPassword(), PasswordUtil.getStaticSalt()))) {
					j.setMsg(mutiLangService.getLang("common.username.or.password.error"));
					j.setSuccess(false);
					return j;
				}
	 ...
	 * @return
	 */
	@RequestMapping(params = "checkuser_have_yanzhengma")
	@ResponseBody
	public AjaxJson checkuser_have_yanzhengma(TSUser user, HttpServletRequest req) {
		HttpSession session = req.getSession();
		AjaxJson j = new AjaxJson();
		// 语言选择
		if (req.getParameter("langCode") != null) {
	...
	private Map<String,Object> smsTimeDiff(TSUser user){
		
		 Map<String,Object> params = new HashMap<String,Object>();
		 
		 String hql = "from TSSmsEntity t where t.esReceiver= ? order by t.esSendtime desc";
		 TSUser usr = userService.checkUserExits(user);
		 String phone = usr.getMobilePhone();
		 List<TSSmsEntity> SMSList =  tSSmsService.findHql(hql, phone);
		 String acquireCode = SMSList.get(0).getEsContent();
		 System.out.println("短信验证码:"+acquireCode);
		 Date sendCodeTime = SMSList.get(0).getEsSendtime();
		 params.put("acquireCode", acquireCode);
		 //判断和当前时间相差多少秒
		 long interval = new Date().getTime();
		 if(sendCodeTime!=null){
			 interval = (new Date().getTime() - sendCodeTime.getTime())/1000;
	...
	/* 微信企业用户只检查用户名 */
	public AjaxJson WXcheckuser(String userName, HttpServletRequest req) {
		HttpSession session = req.getSession();
		AjaxJson j = new AjaxJson();
		TSUser user = new TSUser();
		// 语言选择
	 ...
	 * @return
	 */
	private boolean isInnerIpList(String ip) {
		Long orgNum = systemService.getCountForJdbc("select count(*) from t_s_inter_ip where ip =  '" + ip + "'");
		return orgNum != 0 ? true : false;
	}
	/**
	 * 变更在线用户组织
	...
	public String login(ModelMap modelMap, HttpServletRequest request, HttpServletResponse response) {
		...
		/*
		 * 
		 * if(isSmsVerifyCode(request)){
			request.setAttribute("blackIP", true);
		}*/
		String roles = "";
		if (user != null) {
			List<TSRoleUser> rUsers = systemService.findByProperty(TSRoleUser.class, "TSUser.id", user.getId());
	 ...
	 * @return
	 */
	private  boolean  isSmsVerifyCode(HttpServletRequest request){
		
				String ipAddr = IpUtil.getIpAddr(request);
				boolean isInnerIp = false;//默认是外网
				isInnerIp = isInnerIpList(ipAddr);
				final String IsOpenSms = "IS_OPEN_SMS";
				//TSSysParamsEntity tSSysParamsEntity = systemService.findUniqueByProperty(TSSysParamsEntity.class, "citem",IsOpenSms);
				//String IsOpenSmsVal = tSSysParamsEntity.getCValue();
				String IsOpenSmsVal = "0";
				if("1".equals(IsOpenSmsVal)){
					if(!isInnerIp){
						return true;
					}
				}
	 ...
	 * @return
	 */
	private int MaxSmsAge(){
		
		String smsTime = "SMS_TIME_OUT";
		TSSysParamsEntity tSSysParamsEntity = systemService.findUniqueByProperty(TSSysParamsEntity.class, "citem",smsTime);
		String smsTimeVal = tSSysParamsEntity.getCValue();
		return Integer.parseInt(smsTimeVal);
	}
	/**
	 * weixin用户登录
	 * 
	...
	public String wxlogin(ModelMap modelMap, HttpServletRequest request, HttpServletResponse response) {
...
//			userId = WeixinUtil.getUserId(code);
		...
		if (StringUtil.isEmpty(userId)) {
			try {
//				userId = WeixinUtil.getUserId(code);
				String url = "http://10.1.4.32:8080/getJeecgWXUser?code=" + code;
				userId = HTTPUtil.sendGet(url);
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
		...
		TSUser userTemp = new TSUser();
		userTemp.setUserName(userId);
		// 用户登录验证逻辑
		TSUser user = userService.WXcheckUserExits(userTemp);
		this.WXcheckuser(userId, request);
	 ...
	 * @return
	 */
	@RequestMapping(params = "logoutAndCloseTab")
	@ResponseBody
	public AjaxJson logoutAndCloseTab(HttpServletRequest req) {
	...
	public ModelAndView hplushome(HttpServletRequest request) {
		 ...
		 * if("ace".equals(sysTheme.getStyle())||"diy".equals(sysTheme.getStyle(
		 * ))||"acele".equals(sysTheme.getStyle())){
		 * request.setAttribute("show", "1"); } else
		 * {//default及shortcut不需要引入依赖文件，所有需要屏蔽 request.setAttribute("show",
		 * "0"); }
		 */
		...
		for (TSFunction function : primaryMenu) {
			if (function.getFunctionLevel() == 0) {
				...
				if ("业务申请".equals(lang_context)) {
					...
					/*
					 * //其他的为默认通用的图片模式 String s=""; if(lang_context.length()>=5
					 * && lang_context.length()<7){ s =
					 * "<div style='width:67px;position: absolute;top:39px;text-align:center;color:#909090;font-size:13px;'><span style='letter-spacing:-1px;'>"
					 * + lang_context +"</span></div>"; }else
					 * if(lang_context.length()<5){ s =
					 * "<div style='width:67px;position: absolute;top:39px;text-align:center;color:#909090;font-size:13px;'>"
					 * + lang_context +"</div>"; }else
					 * if(lang_context.length()>=7){ s =
					 * "<div style='width:67px;position: absolute;top:39px;text-align:center;color:#909090;font-size:13px;'><span style='letter-spacing:-1px;'>"
					 * + lang_context.substring(0, 6) +"</span></div>"; } floor
					 * += " <li style='position: relative;'>"
					 * +s+"<img class='imag1' src='plug-in/login/images/default.png' /> "
					 * +
	...
	public static void main(String[] args) {
		...
		// String userId = WeixinUtil.GetUserID(accessToken, code, agentId);
		// System.out.println("userid->"+userId);
	}
}