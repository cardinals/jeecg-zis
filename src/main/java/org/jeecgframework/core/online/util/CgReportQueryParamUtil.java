...

{
		...
		if ("single".equals(queryMode)) {
			...
			try {
				if (StringUtil.isEmpty(value)) {
					value = request.getParameter(filedName.toUpperCase());
					if (StringUtil.isEmpty(value)) {
						return;
					}
				}
			...
			sql_inj_throw(value);
			value = applyType(filedType, value,databaseType);
			if (!StringUtil.isEmpty(value)) {
				if (value.contains("*")) {
					// 模糊查询
			...
			sql_inj_throw(begin);
			begin = applyType(filedType, begin,databaseType);
			String end = request.getParameter(filedName + "_end");
			sql_inj_throw(end);
			end = applyType(filedType, end,databaseType);
	 ...
	 * @return
	 */
	public static String applyType(String fieldType, String value,String databaseType) {
		if (!StringUtil.isEmpty(value)) {
			String result = "";
			if (CgReportConstant.TYPE_STRING.equalsIgnoreCase(fieldType)) {
				...
				result = getDateFunction(value, "yyyy-MM-dd",databaseType);
	 ...
	 * @return 日期函数
	 */
	public static String getDateFunction(String dateStr, String dateFormat,String databaseType) {
